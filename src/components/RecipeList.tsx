...
import { searchRecipes, formatSearchSummary, type RecipesResponse } from "@/services/recipe.service";
import Card from "@/components/ui/Card";
import Button from "@/components/ui/Button";
import BackButton from "@/components/ui/BackButton";
import { WarningIcon, ArrowRightIcon, BackIcon, RefreshIcon } from "@/components/icons/IconCollection";
...
const NoRecipesFound = () => (
  <div className="text-center py-16 px-4">
    ...
    <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">No recipes found</h2>
...
const RecipeCard = ({ recipe }: { recipe: { id: number; title: string; image: string } }) => {
  const imageContent = (
    <>
      <Image
        src={recipe.image}
...
const SearchHeader = ({ searchSummary, totalResults }: { searchSummary: string; totalResults: number }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-8">
    <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
      <div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Search results for</p>
...
export default async function RecipeList({ query, cuisine, maxReadyTime }: RecipeListProps) {
  ...
  const data: RecipesResponse = await searchRecipes({query, cuisine, maxReadyTime: maxTime});
  ...
  const searchSummary = formatSearchSummary({query, cuisine, maxReadyTime: maxTime});

  return (
    <div>
      <SearchHeader searchSummary={searchSummary} totalResults={data.totalResults} />
